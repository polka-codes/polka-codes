...
import { createContext, type ToolHandler, type ToolSignature, type WorkflowFnV2 } from './workflow'
...
const simpleWorkflow: WorkflowFnV2<string, string, Tools> = async function simple(input, { step: _step }) {
  return `hello ${input}`
}

const singleToolWorkflow: WorkflowFnV2<number, number, Tools> = async function singleTool(input, { step: _step, toolHandler }) {
  const result = await toolHandler('add', { a: input, b: 2 })
  return result
}

const multiToolWorkflow: WorkflowFnV2<string, string, Tools> = async function multiTool(input, { step: _step, toolHandler }) {
  const r1 = await toolHandler('concat', { a: 'hello', b: ' ' })
  const r2 = await toolHandler('concat', { a: r1, b: input })
  return r2
}

const failingWorkflow: WorkflowFnV2<null, null, Tools> = async function failing() {
  throw new Error('test error')
}

const failingAfterToolWorkflow: WorkflowFnV2<number, number, Tools> = async function failingAfterTool(input, { step: _step, toolHandler }) {
  await toolHandler('add', { a: input, b: 2 })
  throw new Error('failed after tool')
}

const mockToolHandler: ToolHandler<Tools> = async (tool, input) => {
  switch (tool) {
    case 'add': {
      const addInput = input as Tools['add']['input']
      return addInput.a + addInput.b
    }
    case 'concat': {
      const concatInput = input as Tools['concat']['input']
      return concatInput.a + concatInput.b
    }
  }
...
test('should execute step for each call', async () => {
  ...
  const stepWorkflow: WorkflowFnV2<null, string, Tools> = async function stepWorkflow(_input, { step }) {
...
test('should support steps in a loop', async () => {
  ...
  const loopWorkflow: WorkflowFnV2<number, string, Tools> = async function loopWorkflow(input, { step }) {
