...
async function applyEditOperation(fileContent: string, operation: EditOperation, originalLines: string[]): Promise<string> {
  ...
  if (before_text === START_OF_FILE) {
    // Insert at start or replace from start to after_text
    if (!after_text) {
  ...
  if (after_text === END_OF_FILE) {
    // Insert at end or replace from before_text to end
    if (!before_text) {
  ...
  // Handle normal case with both before_text and after_text
  ...
  // Handle case with only before_text (insert after)
  if (before_text) {
    const beforeIndex = findTextWithHint(fileContent, before_text, before_text_line_start, originalLines)
    const beforeEndIndex = beforeIndex + before_text.length
  ...
  // Handle case with only after_text (insert before)
  if (after_text) {
    const afterIndex = findTextWithHint(fileContent, after_text, after_text_line_start, originalLines)
