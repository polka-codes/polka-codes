...
import { createContext, makeStepFn, type ToolSignature, type WorkflowFn, type WorkflowTools } from './workflow'
...
const singleToolWorkflow: WorkflowFn<number, number, Tools> = async function singleTool(input, { step: _step, tools }) {
  const result = await tools.add({ a: input, b: 2 })
  return result
}

const multiToolWorkflow: WorkflowFn<string, string, Tools> = async function multiTool(input, { step: _step, tools }) {
  const r1 = await tools.concat({ a: 'hello', b: ' ' })
  const r2 = await tools.concat({ a: r1, b: input })
  return r2
}
...
const failingAfterToolWorkflow: WorkflowFn<number, number, Tools> = async function failingAfterTool(input, { step: _step, tools }) {
  await tools.add({ a: input, b: 2 })
  throw new Error('failed after tool')
}

const mockToolHandler: WorkflowTools<Tools> = {
  add: async (input) => {
...
test('makeStepFn should retry failing step', async () => {
  let attempts = 0
  const mock = {
    innerFn: async () => {
      attempts++
...
test('makeStepFn should fail after max attempts', async () => {
  const mock = {
    innerFn: async () => {
