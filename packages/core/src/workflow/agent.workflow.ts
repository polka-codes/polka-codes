...
function isValidToolResultOutput(value: unknown): value is LanguageModelV3ToolResultOutput {
  return (
    ...
    value !== null &&
    'type' in value &&
    (value.type === 'text' || value.type === 'content' || value.type === 'execution-denied')
  )
}
...
export const agentWorkflow: WorkflowFn<AgentWorkflowInput, ExitReason, AgentToolRegistry> = async (input, { step, tools, logger }) => {
  ...
  for (let i = 0; i < maxToolRoundTrips; i++) {
    ...
    const toolResults: { toolCallId: string; toolName: string; output: LanguageModelV3ToolResultOutput }[] = []
