 ...
 many more context lines`,
  ...
  it('should include line numbers when requested', async () => {
    ...
    if (result.type === ToolResponseType.Reply) {
      expect(result.message).toContain('[Line 11 removed]')
  ...
  it('should handle no diff found', async () => {
    ...
    expect(result.type).toBe(ToolResponseType.Reply)
    if (result.type === ToolResponseType.Reply) {
      expect(result.message).toBe('No diff found.')
    }
  })
  ...
  it('should handle missing executeCommand', async () => {
    ...
    expect(result.type).toBe(ToolResponseType.Error)
    if (result.type === ToolResponseType.Error) {
      expect(result.message).toBe('Not possible to execute command. Abort.')
    }
  })
})
