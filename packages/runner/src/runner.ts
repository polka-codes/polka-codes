...
export class Runner {
  ...
  private async handleMessage(message: WsIncomingMessage): Promise<void> {
    if (process.env.DEBUG) {
      console.log('[Runner][DEBUG] Incoming message:', message)
    }
          ...
          if (request.params.overridenAgent === 'coder' && this.provider.executeCommand) {
            ...
            if (foramt) {
              try {
                ...
                console.warn(`[Runner][WARN] Failed to format code using command: ${foramt}`, error)
              }
            }
            if (check) {
              try {
                ...
                console.warn(`[Runner][WARN] Failed to check code using command: ${check}`, error)
              }
            }
            if (test) {
              try {
                ...
                console.warn(`[Runner][WARN] Failed to test code using command: ${test}`, error)
              }
            }
            return {
              type: 'exit',
