...
import { agentWorkflowV2, type WorkflowContextV2 } from '@polka-codes/workflow'
...
describe('commitWorkflow', () => {
  ...
  test('should generate commit message with staged files', async () => {
    ...
    ;(agentWorkflowV2 as any).mockResolvedValue({ type: ToolResponseType.Exit, object: { commitMessage: 'feat: add newFunc' } })
    ...
    expect(agentWorkflowV2).toHaveBeenCalledWith(
  ...
  test('should auto-stage all files when all=true', async () => {
    ...
    ;(agentWorkflowV2 as any).mockResolvedValue({ type: ToolResponseType.Exit, object: { commitMessage: 'feat: stage file' } })
    ...
    expect(agentWorkflowV2).toHaveBeenCalled()
    expect(toolHandler.createCommit).toHaveBeenCalledWith({
