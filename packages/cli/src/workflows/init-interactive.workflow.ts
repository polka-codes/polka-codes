...
> = async (input, context) => {
  ...
  if (generateScript && scriptName && scriptInstructions) {
    const scriptResult = await step('generate-script', async () => {
      ...
      // Note: askFollowupQuestion will be used by the AI agent during generation
      return await agentWorkflow(
        {
...
4. Generate a complete, working TypeScript script`,
    ...
    if (scriptResult.type === 'Exit' && scriptResult.object) {
      ...
      // Show plan and confirm if not skipped
      if (!skipConfirmation) {
        logger.info('\n📋 Script Generation Plan:')
        logger.info(plan)

        // Simple console confirmation for now
        // In a full implementation, this could use the provider's askFollowupQuestion
        logger.info('\n✅ Generating script...')
        logger.info('(To cancel, press Ctrl+C)')
      }
