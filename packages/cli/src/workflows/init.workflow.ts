...
export const initWorkflow: WorkflowFnV2<InitWorkflowInput, InitWorkflowOutput, CliToolRegistry> = async (input, context) => {
  ...
  if (shouldAnalyze) {
    generatedConfig = await step('analyze-project', async () => {
      ...
      return response ? parse((response as { yaml: string }).yaml) : {}
    })
  }
