  ...
  if (pr) {
    ...
    const allChangedFiles: FileChange[] = await step('Getting file changes...', async () => {
      const diffResult = await tools.executeCommand({
        command: 'git',
        args: ['--no-pager', 'diff', '--name-status', '-z', '--no-color', prCommitRange],
      })
      ...
      const statResult = await tools.executeCommand({
        command: 'git',
        args: ['--no-pager', 'diff', '--numstat', '-z', '--no-color', prCommitRange],
      })
    ...
    const allRangeChangedFiles = await step('Getting file changes...', async () => {
      const diffResult = await tools.executeCommand({
        command: 'git',
        args: ['--no-pager', ...diffSelection, '--name-status', '-z', '--no-color', '--'],
      })
      if (diffResult.exitCode !== 0) {
        logger.warn('Warning: Could not retrieve file changes list')
        return []
      ...
      const statResult = await tools.executeCommand({
        command: 'git',
        args: ['--no-pager', ...diffSelection, '--numstat', '-z', '--no-color', '--'],
      })
      if (statResult.exitCode === 0) {
        const stats = parseGitDiffNumStat(statResult.stdout)
        for (const file of files) {
    ...
    const statusResult = await tools.executeCommand({ command: 'git', args: ['status', '--porcelain=v1', '-z', '--untracked-files=all'] })
    const gitStatus = statusResult.stdout
    const statusLines = parseGitPorcelain(gitStatus)
    const hasLocalChanges = statusLines.length > 0

    if (hasLocalChanges) {
      const hasStagedChanges = statusLines.some((line) => line.indexStatus !== ' ' && line.indexStatus !== '?')
      const hasUnstagedChanges = statusLines.some((line) => line.workingTreeStatus !== ' ' && line.workingTreeStatus !== '?')
      const hasUntrackedChanges = statusLines.some((line) => line.indexStatus === '?')
      ...
      const stagedStatResult = await tools.executeCommand({
        command: 'git',
        args: ['diff', '--numstat', '-z', '--cached', '--no-color'],
      })
      ...
      const allBranchChangedFiles = await step('Getting file changes...', async () => {
        const diffResult = await tools.executeCommand({
          command: 'git',
          args: ['--no-pager', 'diff', '--name-status', '-z', '--no-color', branchCommitRange],
        })
        if (diffResult.exitCode !== 0) {
          logger.warn('Warning: Could not retrieve file changes list')
          return []
        ...
        const statResult = await tools.executeCommand({
          command: 'git',
          args: ['--no-pager', 'diff', '--numstat', '-z', '--no-color', branchCommitRange],
        })
        if (statResult.exitCode === 0) {
          const stats = parseGitDiffNumStat(statResult.stdout)
          for (const file of files) {
