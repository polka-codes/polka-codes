...
import type { FullToolInfo, JsonSchema, Logger, McpServerConfig, ToolResponse } from '@polka-codes/core'
...
export class McpManager {
  ...
  async connectToServer(serverName: string, serverConfig: ConfigMcpServerConfig): Promise<void> {
    ...
    let client: IMcpClient | undefined
    try {
  ...
  getFullToolInfos(): FullToolInfo[] {
    ...
    for (const [fullToolName, { tool }] of this.tools.entries()) {
      try {
        const zodSchema = convertJsonSchemaToZod(tool.inputSchema as JsonSchema)
