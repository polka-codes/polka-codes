...
async function createSkill(name: string, logger: Logger, interactive: boolean) {
  ...
  logger.info(`  2. Add only supporting files referenced by the instructions`)
  logger.info(`  3. Validate with: polka skills validate ${name}`)
  logger.info(`  4. Test by asking Polka Codes to use the skill`)
}
...
function generateSkillTemplate(name: string): string {
  ...
  return `---
name: ${name}
description: What this skill does and the specific requests that should trigger it.
---

# ${capitalizedName}

Give Polka Codes the minimum complete workflow for this skill. Reference supporting files only when they add information needed at execution time.
...
export async function main(args: string[]) {
  console.log('Running ${name} with:', args)
}
