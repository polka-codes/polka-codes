...
describe('git_diff tool contract', () => {
  ...
  test.each(
    ["a file's name.txt", 'literal[ab].txt', '--flag.txt', ' padded name.txt '].flatMap((file) =>
      ['.', 'nested'].map((cwd) => ({ file, cwd })),
    ),
  )('keeps revisions and the repository-relative filename literal: %j', async ({ file, cwd }) => {
    ...
    try {
      ...
      process.chdir(join(directory, cwd))
