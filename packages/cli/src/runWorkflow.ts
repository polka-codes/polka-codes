...
export async function runWorkflow<TInput, TOutput, TTools extends ToolRegistry>(
  ...
  try {
    ...
    if (!stateless && memoryConfig.enabled && memoryConfig.type === 'sqlite') {
      ...
      // The store validates the path before creating its directory or lock file.
      sqliteMemoryStore = new SQLiteMemoryStore(
        { enabled: true, type: 'sqlite', path: memoryConfig.path || DEFAULT_MEMORY_CONFIG.path },
        scope,
      )
    }
  } catch (error) {
