...
test('continuous discovery observes uncommitted fixes and new failures with unchanged HEAD', async () => {
  ...
  try {
    ...
    await writeFile(join(dir, 'package.json'), JSON.stringify({ scripts: { typecheck: 'true', build: 'bun build.ts', lint: 'true' } }))
