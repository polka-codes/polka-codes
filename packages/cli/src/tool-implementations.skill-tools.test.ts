...
describe('Tool Implementations - Skill Tools', () => {
  const testDir = join(tmpdir(), `polka-test-skill-tools-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`)
...
`
  ...
  describe('initializeSkillContext', () => {
    it('should initialize with discovered skills', async () => {
      ...
      // Check that our test skill is included (may also include personal/plugin skills)
      const skillNames = context.availableSkills.map((s) => s.metadata.name)
      expect(skillNames).toContain('test-skill')
      // Verify test skill is from project source
      const testSkill = context.availableSkills.find((s) => s.metadata.name === 'test-skill')
