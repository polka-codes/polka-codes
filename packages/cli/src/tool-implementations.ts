...
async function executeCommand(input: CliToolRegistry['executeCommand']['input']) {
  ...
  return new Promise<{ exitCode: number; stdout: string; stderr: string }>((resolve, reject) => {
    const child =
      input.shell === true
        ? spawn(input.command, { shell: true, stdio: ['ignore', 'pipe', 'pipe'], signal: input.signal })
        : spawn(input.command, input.args, { shell: false, stdio: ['ignore', 'pipe', 'pipe'], signal: input.signal })
      ...
      if (APICallError.isInstance(requestError) && requestError.statusCode !== undefined) {
        ...
        if (providerError.retryable && (statusCode === 429 || statusCode >= 500)) {
          ...
          if (hasAnotherAttempt) {
            ...
            await delay(backoff * 1000, undefined, { signal: input.signal })
          }
          continue
        }
